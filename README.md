...
提供雾蓝深色、霜白浅色与暖纸三套外观，使用系统字体栈，兼顾长文阅读与移动端访问。首次访问默认雾蓝深色，不跟随系统外观。导航区域及文章阅读设置中采用“图标 + 当前模式名称”的按钮，点击依次切换暗色、浅色、暖纸。选择保存在当前浏览器，刷新、跨页和再次访问时保持，并同步同源标签页。旧的“跟随系统”偏好或无效数据回退为深色。浏览器禁止存储时，切换仍对当前页面有效；JavaScript 不可用时隐藏切换按钮，保持深色基础阅读。
