...
提供雾蓝深色、霜白浅色与暖纸三套外观，使用系统字体栈，兼顾长文阅读与移动端访问。首次访问跟随系统；读者可通过导航区域的“外观”选择框切换。手动选择保存在当前浏览器，刷新、跨页和再次访问时保持，并同步同源标签页；选择“跟随系统”后恢复自动切换。浏览器禁止存储时，切换仍对当前页面有效。JavaScript 不可用时隐藏选择框，通过 CSS 跟随系统外观。

- **首页**：个人信息与导航、紧凑的介绍区、最新 6 篇文章信息流，以及搜索、年份归档、分类和标签发现栏。介绍区标题按屏幕宽度自然换行，提供“开始阅读”按钮与“了解更多”文字链接。
...
| `_sass/themes.scss` | 三套主题变量、外观选择控件和代码高亮配色 |
| `_sass/reading.scss` | 阅读设置面板、字号、宽度与专注布局 |
| `assets/js/reading.js` | 首屏阅读偏好恢复、设置记忆与段落位置保持 |
| `assets/js/theme.js` | 首屏主题恢复、系统偏好监听与本地选择记忆 |
